...
const UserAllPosts = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 p-4 rounded-2xl bg-gradient-to-r from-muted/50 to-muted/30 border border-border/50">
        {/* Top row - Stats and New Post */}
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-primary/10">
              <span className="text-base sm:text-lg font-bold text-primary">{posts.length}</span>
            </div>
            <div>
          ...
          <Link href="/dashboard/createpost">
            <motion.button
              ...
              whileTap={{ scale: 0.98 }}
              className="inline-flex items-center gap-2 px-3 sm:px-4 py-2 rounded-xl bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 transition-colors shadow-sm"
            >
        ...
        {/* Bottom row - Filter, Sort, View Toggle */}
        <div className="flex flex-col sm:flex-row sm:items-center gap-3">
          {/* Filter tabs - scrollable on mobile */}
          <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0 -mx-1 px-1 scrollbar-none">
            <div className="flex items-center p-1 rounded-xl bg-muted/50 border border-border/50 shrink-0">
              <button
                onClick={() => setFilterBy('all')}
                className={`px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${filterBy === 'all' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >
                All ({posts.length})
              </button>
              <button
                onClick={() => setFilterBy('published')}
                className={`px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${filterBy === 'published' ? 'bg-background shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
              >
                Published ({publishedCount})
              </button>
              <button
                onClick={() => setFilterBy('drafts')}
                className={`px-2.5 sm:px-3 py-1.5 rounded-lg text-xs font-medium transition-all whitespace-nowrap ${filterBy === 'drafts' ? 'bg-background shadow-sm text-amber-600 dark:text-amber-400' : 'text-muted-foreground hover:text-foreground'}`}
              >
                Drafts ({draftsCount})
              </button>
            </div>
          ...
          <div className="flex items-center gap-2 ml-auto">
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none px-2.5 sm:px-3 py-2 pr-7 sm:pr-8 rounded-xl border border-border bg-background text-xs sm:text-sm font-medium text-foreground cursor-pointer hover:border-primary/50 transition-colors focus:outline-none focus:ring-2 focus:ring-primary/20"
              >
            ...
            <div className="flex items-center p-1 rounded-xl bg-muted/50 border border-border/50">
              <button
                ...
                className={`p-1.5 sm:p-2 rounded-lg transition-all ${viewMode === 'list' ? 'bg-background shadow-sm text-primary' : 'text-muted-foreground hover:text-foreground'}`}
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              ...
              <button
                ...
                className={`p-1.5 sm:p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-background shadow-sm text-primary' : 'text-muted-foreground hover:text-foreground'}`}
              >
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          >
            {filteredPosts.map((post, index) => (
              <motion.div
                ...
                className="group"
              >
                <div className="relative flex flex-col sm:flex-row gap-3 sm:gap-4 p-3 sm:p-4 rounded-2xl border border-border bg-card hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300">
                  {/* Left accent bar */}
                  <div className={`absolute left-0 top-3 sm:top-4 bottom-3 sm:bottom-4 w-1 rounded-full ${post.published === false ? 'bg-gradient-to-b from-amber-500/60 via-amber-500/30 to-transparent' : 'bg-gradient-to-b from-primary/60 via-primary/30 to-transparent'} opacity-0 group-hover:opacity-100 transition-opacity duration-300`} />
                  
                  {/* Thumbnail */}
                  <Link href={`/blog/${post.slug}`} className="shrink-0">
                    <div className="relative w-full sm:w-40 md:w-44 h-24 sm:h-28 rounded-xl overflow-hidden bg-muted">
                      <img
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    <div>
                      <Link href={`/blog/${post.slug}`}>
                        <h3 className="font-semibold text-foreground text-sm sm:text-base line-clamp-2 sm:line-clamp-1 group-hover:text-primary transition-colors">
                          {post.title}
                        </h3>
                      </Link>
                      ...
                      <div className="flex items-center gap-2 sm:gap-3 mt-1.5 sm:mt-2 text-xs text-muted-foreground">
                        ...
                        <span className="hidden sm:block text-muted-foreground/70 truncate max-w-[180px]">/{post.slug}</span>
                      </div>
                    </div>

                    {/* Bottom row - Actions */}
                    <div className="flex items-center justify-between mt-2.5 sm:mt-3 pt-2.5 sm:pt-3 border-t border-border/50">
                      <div className="flex items-center gap-1">
                        <Link href={`/blog/${post.slug}`}>
                          <motion.button
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted/80 transition-colors"
                          >
                            <svg className="w-3 sm:w-3.5 h-3 sm:h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                      ...
                      <div className="flex items-center gap-1">
                        <motion.button
                          ...
                          className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-500/10 hover:bg-blue-500/20 transition-colors"
                        >
                          <svg className="w-3 sm:w-3.5 h-3 sm:h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                        ...
                        <motion.button
                          ...
                          className="inline-flex items-center gap-1 sm:gap-1.5 px-2 sm:px-3 py-1.5 rounded-lg text-xs font-medium text-destructive bg-destructive/10 hover:bg-destructive/20 transition-colors disabled:opacity-50"
                        >
                          {deleteId === post._id ? (
                            <div className="w-3 sm:w-3.5 h-3 sm:h-3.5 border-2 border-destructive/30 border-t-destructive rounded-full animate-spin" />
                          ) : (
                            <svg className="w-3 sm:w-3.5 h-3 sm:h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
